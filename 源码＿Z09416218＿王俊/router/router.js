...
exports.Get_login = (request, response, next) => {//获取登陆界面
	response.render("login");
}

exports.Post_login = (request, response, next) => {//提交登陆信息
...
exports.Post_addThing = (request, response, next) => {//添加计划
	if(request.session.zhanghao == null) {
		response.writeHead(302, {
			"Location": "http://127.0.0.1:3000/login"
		});
	...
	form.parse(request, (err, fields, files) => {
		...
		var params = [title,message,urgent,time,list_id,0];
...
exports.Get_delete = (request,response,next)=>{//删除计划,软删除方式
	if(request.session.zhanghao == null) {
		response.writeHead(302, {
			"Location": "http://127.0.0.1:3000/login"
		});
